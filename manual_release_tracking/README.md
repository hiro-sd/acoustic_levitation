...
4. 最初の明確な離反を `RELEASE_CANDIDATE` とした時点で、捕捉専用の
   3D運動履歴をリセットします。この観測中は音場を出しません。接触へ
   戻った場合は候補と観測履歴を破棄します。
5. release確定に加えて、release候補より後の新規ステレオ測定が最低5フレーム、
   20 ms以上揃ってから初期捕捉を開始します。直近最大60 msのXYZ位置を
...
6. `CAPTURE_ALIGN` 中は現在のZ速度を120 msで停止させるための必要力を
   ...
   `CAPTURE_SETTLE`へ移行しません。120 ms後もZ制動が完了していない場合、
   XYは軌道終点に固定せず、その終点を基準とする通常PIDへ切り替えます。
   Zはその間も従来の制動軌道を継続します。
