...
def main():
    ...
    try:
        with Controller.open(
            ...
            # --- ここで一度だけ送信 (ループの外) ---
            autd.send(Silencer())
            m = Static(intensity=int(0xFF * 0.8))  # 振幅変調を行わず、常に同じ振幅を出力する (小球は* 0.6)
            ...
            # 送信！ (これを一度送れば、デバイス側でずっと円運動してくれます)
            autd.send((m, stm))
            print("[INFO] STM sent. Starting vision loop...")

            # --- メインループ (画像認識のみ) ---
            window_name = "YOLO Vision Check"
            ...
            while True:
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        if (target_cls_id is None) or (cls_id == target_cls_id):
                            cv2.rectangle(annotated, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2)
                            label_text = f"{model.model.names[cls_id]} {score:.2f}"
