...
5. release確定に加えて、release候補より後の新規ステレオ測定が最低4フレーム、
   ...
   実測したAUTD送信遅延13 msを考慮した予測位置へ音場中心を合わせます。
...
6. `CAPTURE_ALIGN` 中は現在のZ速度を120 msで停止させるための必要力を
   ...
   一定減速の `z_ref(t), v_ref(t)` を追従します。XYもrelease時の初期速度を
   120 msで0にする一定減速軌道を使います。球が初速度のまま等速移動する
   位置より音場を徐々に後方へ遅らせ、横方向の復元力で制動する設計です。
   初期XY速度の大小にかかわらず同じ120 ms軌道を使用し、軌道完了前には
   `CAPTURE_SETTLE`へ移行しません。120 ms後もXY速度が60 mm/sを超える、
   または球と音場中心の距離が10 mmを超える場合は、通常PIDへは渡さず、
   球の移動方向に対して最大12 mm後方へ音場中心を置く速度制動を続けます。
   XY速度60 mm/s以下かつ距離10 mm以下が20 ms続いた時点の球位置を
   一時基準として通常PIDへ切り替えます。また、`CAPTURE_ALIGN`と
   `CAPTURE_SETTLE`では、最終的に送る音場中心を球から15 mm以内に制限し、
   PIDの微分項などで音場の影響範囲外へ離れることを防ぎます。
   Zはその間も従来の制動軌道を継続します。
